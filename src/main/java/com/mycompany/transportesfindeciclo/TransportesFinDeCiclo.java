...
// Importaciones corregidas con las mayúsculas correctas en DataSources
...
public class TransportesFinDeCiclo {
    ...
    public static void main(String[] args) {
        ...
        DaoEnvios fuenteEnvios = new Envios();

        Vehiculo auto = new Carro(1, "Hino", "GD 2026", 1200, "Carro de carga pesada");
        Vehiculo moto = new Moto(2, "Shineray", "XY200", 90, 2000); 
        Vehiculo aeroplano = new Avion(3, "Airbus", "330", 70000, "Avion de carga pesada", 20000);

        fuenteVehiculos.guardar(auto);
        fuenteVehiculos.guardar(moto);
        fuenteVehiculos.guardar(aeroplano);

        System.out.println("Metodos de trsnaportes");
        auto.transportar();
        moto.transportar();
        aeroplano.transportar();

        System.out.println("CalLCULO las distancias recorridas");
        ...
        double distAvion = aeroplano.calcularDistancia(800.0, 2.0, 1.2);
        ...
        if (fuenteEnvios.guardar(envio1)) {
